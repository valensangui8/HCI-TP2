<!--<template>
  <v-dialog v-model="showModal" width="50%" persistent>
    <v-card>
      ...
      <v-card-text>
        <form @submit.prevent class="form">
          ...
          <div v-if="step === 3">
            <div class="component-container d-flex justify-space-between">
              <destinatario
          ...
          <v-btn v-if="step < 3" @click="nextStep" color="primary" class="mt-4">{{ step < 3 ? 'Siguiente' : 'Confirmar' }}</v-btn>
        </form>
      </v-card-text>
  ...
  <v-dialog v-model="confirmationDialog" max-width="400">
    <v-card>
      ...
      <v-card-text>El pago ha sido exitoso. ¿Desea continuar con otra transacción?</v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
...
export default {
  ...
  setup() {
    const router = useRouter();
    const showModal = ref(true); // Set to true to show modal initially
    const step = ref(1);
    const paymentMethod = ref('');
    const paymentDetails = ref('');
    const amount = ref(0);
    ...
    <v-card>
      ...
      <v-card-text>
        <form @submit.prevent>
          ...
          <!-- Step 3: Display Recipient and Amount Input -->
          <div v-if="step === 3">
            <div class="d-flex justify-space-between mb-4">
              <destinatario
                class="flex-grow-1 mr-4"
